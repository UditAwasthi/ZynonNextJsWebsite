...
interface Props {
    ...
    /** Optional: pass threads from parent to avoid a duplicate API call */
    cachedThreads?: Thread[];
}

export default function ForwardModal({ onClose, onForward, cachedThreads }: Props) {
    const [threads, setThreads]   = useState<Thread[]>(cachedThreads ?? []);
    const [query, setQuery]       = useState("");
    const [selected, setSelected] = useState<string[]>([]);
    const [sending, setSending]   = useState(false);
    const [sentIds, setSentIds]   = useState<string[]>([]);
    const [loading, setLoading]   = useState(!cachedThreads?.length);

    // Only fetch if no cached threads were passed
    useEffect(() => {
        if (cachedThreads?.length) return;
        getInbox()
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
            ...
            <div className="relative bg-white dark:bg-zinc-950 w-full max-w-[400px] rounded-[32px] shadow-2xl border border-zinc-200 dark:border-zinc-800 flex flex-col overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-10 duration-300">
                ...
                <div className="px-6 py-5 flex items-center justify-between border-b border-zinc-100 dark:border-zinc-900">
                    <button
                        onClick={onClose}
                        className="p-2 -ml-2 rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-900 text-zinc-500 transition-colors"
                    >
                        <X size={20} />
                    </button>
                ...
                <div className="flex-1 overflow-y-auto max-h-[400px] px-2 pb-4 scrollbar-hide">
                    {loading ? (
                        ...
                        <p className="text-center text-sm text-zinc-400 py-10">No conversations found</p>
                    ) : (
                        filtered.map(thread => {
