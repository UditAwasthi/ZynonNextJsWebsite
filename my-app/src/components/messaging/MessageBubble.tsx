        ...
        >
            ...
            <div className={`flex flex-col ${isMine ? "items-end" : "items-start"} max-w-[85%] sm:max-w-[70%]`}>
                ...
                <div className={`relative flex items-center gap-2 ${isMine ? "flex-row-reverse" : "flex-row"}`}>
                    ...
                    {/* Main Bubble Content */}
                    <div className={`press-effect ${isPressing ? "is-pressing" : ""} relative transition-all duration-300 ${
                        isMine 
                        ...
                        {message.attachments?.map((file, idx) => (
                            <div key={idx} className="p-1">
                                {file.type === "image" ? (
                                    <img src={file.url} alt="Media" className="rounded-[18px] max-h-80 w-full object-cover transition-all" />
                                ) : (
                        ...
                        {message.content && (
                            <div className="px-5 py-3 text-[15px] leading-relaxed font-medium select-none">
                                {message.content}
                            </div>
                        )}
                    </div>
                    ...
                    {showMenu && (
                        <>
                            ...
                            <div className={`absolute bottom-full mb-3 z-50 w-56 bg-white/95 dark:bg-zinc-900/95 menu-blur rounded-[28px] shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-white/20 overflow-hidden animate-in fade-in zoom-in-95 slide-in-from-bottom-4 duration-300 ${isMine ? 'right-0' : 'left-0'}`}>
                                ...
                                <MenuItem icon={<Pin size={18} />} label="Pin" onClick={() => { onPin(message._id); setShowMenu(false); }} />
                                {canEdit && <MenuItem icon={<Edit2 size={18} />} label="Edit" onClick={() => { onEdit(message); setShowMenu(false); }} />}
                                <div className="h-px bg-zinc-100 dark:bg-zinc-800/50 mx-4 my-1" />
                                {canDelete && <MenuItem icon={<Trash2 size={18} />} label="Delete" danger onClick={() => { onDelete(message._id); setShowMenu(false); }} />}
                            </div>
                        </>
                    )}
                </div>
                        ...
                        <div className="flex items-center gap-1 animate-in zoom-in duration-300">
                            {Object.entries(reactionMap).map(([emoji, count]) => (
                                <button 
                                    ...
                                    className={`flex items-center px-2 py-0.5 rounded-full border shadow-sm text-[12px] transition-all hover:scale-110 ${
                                        myReaction === emoji 
                                ...
                                >
                                    <span>{emoji}</span>
                                    {count > 1 && <span className={`ml-1 font-bold ${myReaction === emoji ? 'text-blue-600' : 'text-zinc-500'}`}>{count}</span>}
                                </button>
                            ))}
                        </div>
                    )}
...
function MenuItem({ icon, label, onClick, danger = false }: {
    ...
    return (
        <button
            onClick={onClick}
            className={`w-full flex items-center justify-between px-5 py-3.5 text-[14px] font-bold transition-all active:bg-zinc-100 dark:active:bg-zinc-800 ${
                danger
